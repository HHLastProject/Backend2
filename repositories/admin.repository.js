...
class AdminRepository {
    ...
    getAllShops = async () => {
        const allShops = await Shops.findAll({
            attributes: [ 'shopId', 'shopName', 'category', 'roadAddress', 'jibunAddress', 'x', 'y'],
            order: [['createdAt', 'DESC']]
        });
        return allShops;
    };

    postShop = async (adminId, shopName, category, roadAddress, jibunAddress, x, y, operatingTime, phoneNumber, thumbnail) => {
        console.log (adminId, shopName, category, roadAddress, jibunAddress, x, y, operatingTime, phoneNumber, thumbnail)
        const createdShop = await Shops.create({
            AdminId: adminId, shopName, category, roadAddress, jibunAddress, x, y, operatingTime, phoneNumber, thumbnail
        });
    ...
    updateShop = async (shopId, shopName, category,roadAddress, jibunAddress, x, y, operatingTime, phoneNumber, thumbnail) => {
        const updatedShop = await Shops.update(
            { shopName, category, roadAddress, jibunAddress, x, y, operatingTime, phoneNumber, thumbnail },
            { where: { shopId } }
            );
        return updatedShop;
    };
    ...
    getOneShopInfo = async (shopId) => {
        const shopInfo = await Shops.findOne({
            attributes: ['shopId', 'adminId', 'shopName', 'category', 'roadAddress', 'jibunAddress', 'x', 'y', 'operatingTime', 'phoneNumber', 'thumbnail'],
            where: {shopId},
            include: [
                {
                    model: Menus,
