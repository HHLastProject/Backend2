...
class favoriteController {
  ...
  listFavorite = async (req, res, next) => {
    try {
      ...
      res.status(401).json({ msg: "예기치 못한 오류가 발생했습니다" });
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < folderList.length; i++) {
        for (let j = 0; j < folderList[i].shopList.length; j++) {
          ...
          if (isMyScrapbyshopList == -1) {
            return res
              .status(400)
              .json({ msg: "즐겨찾기에 추가된 목록이 아닙니다" });
          }
        }
      }

      const folderListName = folderList.map((value) => {
        return value.folderName;
      });
      ...
      for (let i = 0; i < folderListName.length; i++) {
        if (existFolderId[i] != folderListName[i]) {
          return res
            .status(400)
            .json({ msg: "기존에 폴더를 수정하여 문제가 발생" });
        }
      }

      await this.favoriteService.listMovePatch(folderList, userId);
  ...
  makeFolder = async (req, res, next) => {
    try {
      ...
      const FolderListNameResult =
        await this.favoriteService.compareFolderListName(folderList);
      if (FolderListNameResult) {
        return res
          .status(400)
          .json({ msg: "동일한 파일 이름을 설정하였습니다" });
      }
      ...
      //리스트를 수정(폴더 이름으로)
      await this.favoriteService.listPatch(createdfolder, existFolderId);
